...
No single SAILORS letter maps to exactly one OWASP item, and that is expected. A gap in access controls (A) can just as easily show up as a data disclosure problem (LLM02) or an identity problem (ASI03), depending on which side of the retrieval you are looking from. SAILORS is not trying to force a clean one-to-one mapping. It is giving you seven places to look. This table just shows what you are likely to find when you look there.

## What is not covered

SAILORS provides direct or partial design-time coverage for most capability-level OWASP LLM and Agentic risks. It does not cover risks that require architecture-level or runtime controls (supply chain, data poisoning, unbounded consumption, inter-agent communication). See the [coverage matrix](docs/owasp-coverage-matrix.md) for what SAILORS covers directly, what it partially covers, and what companion controls fill the gaps.

## What is not here yet

This is a first pass, built from the two OWASP taxonomies and a general read of ATLAS's structure, not from a full technique-by-technique crosswalk. If you are using this table for actual compliance mapping or a formal threat model, verify the ATLAS column against the current framework directly before relying on it.

Corrections and refinements are welcome. That is exactly the kind of contribution this repo is asking for.
