...
That is it. Same model, same data sources, same system prompt.

## Which checks the trigger matrix said to re-run

New tool plus new output sink means re-run **L, O, I**. Not all seven. That is the whole point of the trigger matrix: a small change gets a small review.

I also re-ran **R** as a sanity check on the audit trail, because the refund is a money-moving action and I wanted the log entries in one place. S (both of them) and A did not need re-running.
...
**Fix:** Give the refund tool its own credential, hard-capped at the configured threshold at the API gateway. The model should be architecturally incapable of issuing a refund above the threshold, not just told not to.
...
**Fix:** Log the full chain: customer message, retrieved order data, model output and reasoning, final action. That is the difference between "we do not know what happened" and "here is exactly what happened and why," six weeks later when nobody remembers the ticket.
...
Three checks re-run in about thirty minutes. Two blockers (L and O), one partial that is a blocker for the money path (I), one partial on the audit trail (R).
...
None of these findings are exotic. Scoped credentials, threshold enforcement, output bounds checking, full logging. Standard security practice, applied to a place teams often forget to apply it, because "it is just a chatbot" makes it feel lower-risk than it actually is.

This is what SAILORS is for. The scanner missed I (it does not cover output inspection yet). The AIBOM diff told me which three checks mattered. A human review closed the loop in half an hour.
...
- [`../templates/sailors-review-template.md`](../templates/sailors-review-template.md): the fillable version of what is above.
