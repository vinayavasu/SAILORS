====================
# Worked Example: Support Assistant with Refund Capability (walkthrough)

*Updated July 2026 to reflect two SAILORS revisions from practitioner feedback: O now includes a scope-expansion gate alongside the action gate, and A now includes a MAESTRO hand-off for agent-sourced retrieval. See [CHANGELOG.md](../CHANGELOG.md) for details.*
...
This is a worked example of applying SAILORS to a single AI capability during a design review. It is a review of one feature, not of the whole support platform.
...
**Fix:** Cap message length, strip control characters, and wrap customer input so it is clearly delineated from system instructions in the prompt structure. Embedded instruction-like text needs to be treated as data, never as a command.

---

## A, Access controls at retrieval

This is the one that showed up in almost every review I have run, not just this one. The lookup tool was querying the orders database using a shared service account with read access to all customer orders, not scoped to the requesting customer's session at all.

**Fix:** Scope the query to the authenticated customer's own customer ID at the database layer, not just the prompt layer. The model should never have the *option* to retrieve another customer's order, no matter what it is asked.

**Also check:** Is this retrieval actually static, or is it another agent's output? If a separate fraud-check agent were handing back a verdict on this customer instead of a direct database call, that is no longer a retrieval-permissions question. That is a MAESTRO question, since you would be inheriting that agent's trust level instead of checking a permission directly.
...
**Fix:** Give the refund tool its own scoped credential, hard-capped at the configured threshold, so it is architecturally incapable of issuing a refund above that amount. Not just told not to. Actually unable to.
...
**Also check:** Is there a second gate for scope, not just the action? If this same assistant later gets handed a new capability (address changes, store credit, not just refunds), that is new scope, and it needs its own approval gate. It does not inherit approval from the original refund threshold just because it is the same assistant.
...
**Fix:** Log the full chain: customer message, retrieved order data, model output and reasoning, and the final action taken. That is the difference between "we do not know what happened" and "here is exactly what happened and why," six weeks later when nobody remembers the ticket.
