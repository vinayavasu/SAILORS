...
A filled-in SAILORS review of one AI capability. The full write-up is in examples/refund-tool-review.md.
...
Start by opening templates/sailors-review-template.md and running it against one of your capabilities.
...
How to use it
Threat model the capability (STRIDE or MAESTRO) to identify what can go wrong.

Use the threat model to select the relevant SAILORS checks.

Open the review template.

Walk the relevant checks against the capability.

Flag gaps as findings. Prioritize by exploitability and blast radius.

Record the decision: ship, ship with fixes, or block.

Re-run at each significant AIBOM change. The trigger matrix tells you which checks.

A filled-in review is in examples/refund-tool-review.md.

Review template
The review template is a fillable document for a thirty-minute design review. It has sections for:
...
AIBOM trigger
SAILORS reviews are triggered by changes to your AI Bill of Materials, not by a calendar. The AIBOM trigger document covers:
...
The full mapping is in mapping.md. The coverage matrix — what SAILORS covers directly, what it covers partially, and where companion controls fill the gap — is in docs/owasp-coverage-matrix.md.
...
Praxen — an open-source agent behavior verifier. Reads code and deployment state, checks against a declared policy (Worker Remit), produces a scored posture report with file:line evidence.
...
A comparison of SAILORS against Praxen and RAISE, using a deliberately broken demo agent called FinBot, is in praxen-comparison/.
...
Status
v1.1. Actively seeking practitioner feedback from teams shipping AI capabilities. Release history is in CHANGELOG.md.

Contributing
The most useful contribution right now is a real capability write-up. Anonymize the names. Keep the findings honest. Three concrete ways to help are in CONTRIBUTING.md.
